...
$BuildRoot = Join-Path $RepoRoot ".build\mg-simulator"
$VenvPython = Join-Path $BuildRoot "venv\Scripts\python.exe"
$DistRoot = Join-Path $BuildRoot "dist"
$WorkRoot = Join-Path $BuildRoot "work"
$AppDir = Join-Path $DistRoot "MGSimulator"
$ArtifactDir = Join-Path $RepoRoot "artifacts\mg-simulator"
$Version = "1.0.0"
$ZipPath = Join-Path $ArtifactDir "MGSimulator-$Version-win-x64.zip"
$ChecksumPath = "$ZipPath.sha256"
$InstallerName = "MGSimulator-Setup-$Version-win-x64.exe"
...
try {
    & $VenvPython -m PyInstaller `
        ...
        --distpath $DistRoot `
        --workpath $WorkRoot `
        (Join-Path $PackageDir "MGSimulator.spec")
...
$Assets = @(
    "start-active.bat",
    "start-passive.bat",
    "start-band-refusal-demo.bat",
    "start-host-offline-demo.bat",
    "README_OPERATOR.md",
...
if (-not $AppExecutables) { throw "MG simulator package contains no executable." }
...
$InnoArguments = @(
    ...
    (Join-Path $PackageDir "MGSimulator.iss")
)
